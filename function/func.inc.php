...
function disrupt($username, $request)
{
	//Send Mail
	$date =date("Y-m-d H:i:s");
	$message = "LOGIN ISSUE:: Login from Username: {$username} at: {$date} is not successful ";
	$subject = "LOGIN ISSUE";

	$email = "michael@airvend.ng";
	$email2 = "support@airvend.ng";                                                                                                                                                                                                              $email3 = "calexander@callphoneng.com"; 

	//Send Mail
	sendMail($email, $subject, $message);
	sendMail($email2, $subject, $message);
	sendMail($email3, $subject, $message);
	callSlack($message);
  ...
  //              print_r($row);
    ...
    //                    print_r($query);
			...
			if($type == 1){ return;}
                        ...
                        $email = "michael@airvend.ng";
			$email2 = "support@airvend.ng";
			$email3 = "calexander@callphoneng.com";
                        sendMail($email, $subject, $message);
			sendMail($email2, $subject, $message);
			sendMail($email3, $subject, $message);
                                                                                                                                                                                                        callSlack($message);                                                                                                                                                                                                                 }
...
function sendMail($email, $subj, $mess)
{
	$httpheader = array(
                    "Content-Type: application/json",
                    "Authorization: Bearer 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"
                );


	$data = array("personalizations"=>array( array("to"=>array( array("email"=>$email)))),
	              "from"=>array("email"=>"hello@airvendng.com"),
	              "subject"=>$subj,
	              "content"=>array( array("type"=>"text/html", "value"=>$mess)));
...
function callSlack($information){
   
        //Service Failure
	$url = "https://hooks.slack.com/services/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";
    
	$OAUth = "xoxb-899630719424-1583043643143-dpJZpyD80G4mNwiUkkpxCVM1";
	...
	$ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-type: application/json","Authorization: Bearer {$OAUth}"));
